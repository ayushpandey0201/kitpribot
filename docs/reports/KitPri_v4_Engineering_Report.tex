...
\midrule
Max epochs / batch     & 20 / 16        & 30 / 32       & --- \\
LR / weight decay      & $10^{-4}$ / 0.01 & $3{\cdot}10^{-4}$ / 0.01 & --- \\
Schedule               & cosine         & cosine        & --- \\
Early stop (patience)  & val F1 (5)     & val F1 (6)    & --- \\
Outcome                & best epoch 1, stop @ 6 & best epoch 8, stop @ 14 & 500 calib.\ clips \\
\bottomrule
\end{tabular}
\caption{Training configuration (from \texttt{configs/experiments/*.yaml};
outcomes from \texttt{run\_summary.json}).}
\label{tab:hyper}
\end{table}
...
$\alpha$'s role is inconsistent across the literature; the codebase documents
it as a flagged assumption rather than a silent choice.
